...
class Access
{
    ...
    public static function log(string $requestId) {

        if (self::exists($requestId)) {
            return false;
        }
    ...
    private static function exists(string $requestId): bool {
        $sql = "SELECT 1 FROM nano_requests WHERE request_id = ? LIMIT 1";
        $res = Data::query($sql, [$requestId]);
        return !empty($res);
    }

    private static function request(string $requestId) {
        $sql = "INSERT INTO nano_requests (request_id, created_at) VALUES (?, datetime('now'))";
        Data::query($sql, [$requestId]);
    }
